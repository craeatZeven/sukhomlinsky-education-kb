---
id: sk-0023
aliases:
  - sk-0023
  - 睡前长时间做作业，孩子会开始落后
type: method
title: "睡前长时间做作业，孩子会开始落后"
lang: zh-CN
topics:
  - health-first
  - learning-difficulties
source: on-education
ref: "On Education (Progress Publishers, 1977), EPUB page 118"
url: "https://archive.org/details/vasili-sukhomlinsky-on-education-progress-1977"
status: reviewed
created: "2026-09-06"
updated: "2026-09-06"
reviewed_by: "maintainer (source-checked)"
---

# Card sk-0023 — 睡前长时间做作业，孩子会开始落后

## 原文/Excerpt

> ...if a child sits at his homework for several hours before bedtime, he will start to lag behind. Passivity while a child is nominally engaged in mental work at his lessons is more often than not the result of the fact that a child spends the hours he should be out of doors—in the garden or playing snow balls—poring over books.

## 中文转述/说明

如果孩子把本应户外活动的时间用来熬夜写作业，反而会开始落后；课堂上的“假性用功/被动”往往正是户外时间被挤占的结果。苏霍姆林斯基据此强调作业与作息安排，而不是简单增加学习时长。

## 教育场景/应用

在给学业落后孩子制定计划时，先检查晚间作息：作业是否拖到睡前、户外/睡眠是否被压缩。减少“久坐刷题”，把学习任务前移并保障户外与睡眠，常比加码更有效。

## 出处核对

- source: on-education
- ref: On Education (1977), EPUB page 118
- 已核对原文：是（本地英文文本）

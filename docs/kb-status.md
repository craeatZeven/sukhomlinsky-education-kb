...
**浏览器层回归**（需要真实 Chrome 与静态服务，进不了上面这条离线链）：[`tools/web-audit/`](../tools/web-audit/README.md) —— 24 条行为/数据/对比度判据 + 三套主题 × 七个页面的对比度普查。跑法：`powershell -NoProfile -ExecutionPolicy Bypass -File tools\web-audit\run-audit.ps1`。
...
| 浏览器回归 | 24 条判据全过 + 三主题 × 七页面对比度全过（顺手修掉 dark 主题 `.chip--primary` 2.39:1、paper 主题 `.row-go` 4.36:1） | `tools/web-audit/run-audit.ps1` |

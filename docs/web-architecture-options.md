...
> **尚未做**：Pages 上的静态版仍按原样运行（首屏 755 KB gzip）。
> **Cloudflare 变体（2026-09-09 已上线）**：`cloudflare/`（wrangler.toml + `src/index.js` + `scripts/build_d1_sql.py`）。
> 已部署到 **https://suk-kb-api.suk-kb.workers.dev**（D1 `suk-kb`，1386 卡 + FTS5），线上实测
> `/api/search?q=苏霍姆林斯基`→FTS5 771、`q=劳动`→LIKE 372、`/api/meta`→1386/12/12；
> GitHub Pages 的 `web/search.html` 已连上该后端。**限制**：`*.workers.dev` 在国内被 DNS 污染，
> 脚本类客户端不可达（浏览器可访问），主要受众在大陆需绑自定义域名；`search.html` 已加静态回退。
...
5. **所以「全部内容一个页面」要这样做**：
   ...
   - 首屏体积问题已归**档位 A（静态分片）并已实施**（2026-09-10）：`web/data.js` 3.31 MB / 772 KB gzip 已拆成 `web/data/` 分片，首页 75 KB→gzip 约 28 KB、卡片详情 9.6 KB、浏览页 1.10 MB（索引，gzip 306 KB）。实测与验收见 `docs/web-performance-sharding.md`。

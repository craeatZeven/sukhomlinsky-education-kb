...
- 卡片总数：1186（13 sources / 12 topics）
...
- 五卷本：已按作品统计卡片来源分布（见 `docs/coverage-volumes.md`），并完成章节级覆盖审计（见 `docs/coverage-volumes-chapters.md`）与剩余缺口审计（见 `docs/coverage-volumes-gaps-remaining.md`）：143 个原 gap 中 66 个已不再空白、76 个仍空白、1 个口径存疑；剩余缺口 95% 集中在第2卷

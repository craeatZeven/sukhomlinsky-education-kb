---
id: sk-0002
primary: A5
seealso: []
facets: []
tax_tags:
  - A18
aliases:
  - sk-0002
  - 先让孩子变得“可教”，而不是吼叫与说教
type: method
title: "先让孩子变得“可教”，而不是吼叫与说教"
lang: zh-CN
topics:
  - learning-difficulties
source: on-education
ref: "On Education (Progress Publishers, 1977), p. 34（EPUB page 35）"
url: "https://archive.org/details/vasili-sukhomlinsky-on-education-progress-1977"
status: reviewed
created: "2026-09-06"
updated: "2026-09-06"
reviewed_by: "maintainer (source-checked)"
---

# Card sk-0002 — 先让孩子变得“可教”，而不是吼叫与说教

## 原文/Excerpt

> Children never respond to angry words... Before a child can be educated he has to be rendered educable. Before the teacher addresses himself directly to a child, it is essential that the child should be in a state in which he is capable of taking in the teacher's words... Just as he should refrain from shouting at weak pupils and give them additional help instead, so he should stop himself shouting at pupils who do not pay attention.

## 中文转述/说明

面对“说不动”的孩子，教师常以为问题是孩子不听话；苏霍姆林斯基认为真正要做的是先恢复孩子的“可教育性”——让儿童处于能听进教师话语的状态。对弱生不吼叫，而是给额外帮助；对道德引导也同样，不用惩罚和向家长告状开路。

## 教育场景/应用

适用：课堂纪律差、后进生屡教不改、师生对话失效。
操作顺序：先不急着批评/讲道理 → 判断孩子当前是否具备接收教育的能力 → 用耐心和额外帮助培养这种能力 → 再谈要求。

## 出处核对

- 定位：on-education

- source: on-education
- ref: On Education (1977), p. 34
- 已核对原文：是（Archive.org 英文 OCR 文本）

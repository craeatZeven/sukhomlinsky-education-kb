...
### `verify-hardened.mjs` —— 47 条判据

| 分区 | 条数 | 在防什么 |
|---|---|---|
| 检索 | 3 | 自然问句真的命中对应预设问题、命中卡真的是讲那件事的、总数如实显示（不是把每页 30 当总数） |
| 地图 | 4 | 延迟索引 + 连点两次不错配（竞态）、窄屏与桌面的板块顺序 |
| 顶栏导航（桌面 / 窄屏各 5 条） | 10 | 三组按钮都在且单行不越界、**15 个目的地一个不少**、品牌回首页（本版去掉了单独的「首页」项）、当前页所在**组**高亮、三组**各自**展开时菜单可见/标签单行/整条落在视口内 |
| 分类总图 | 7 | 三句断言的数字与 `meta.json` **独立算一遍**核对（不是页面上写着就算）、5 条泳道条目数与 meta 一致、**版图块宽与张数单调同序**、22 个持卡条目都能点进条目页、**两类卡排在版图之前**（读者优先）、**评估者那块默认收起**（展开前后高度差明显）、窄屏退化成列表且触控 ≥40px 无横溢 |

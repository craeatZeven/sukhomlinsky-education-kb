...
- **新增「分类总图」页（2026-09-14，`web/taxonomy.html`）**：**读者优先**的一页——`web/taxonomy.html` 先回答"我该怎么找"：**两类卡**（论述卡 713 = 他主张了什么 / 故事卡 673 = 发生了什么，各自怎么找）→ 参见是什么（角度级 5×5 + 最该一起看的条目对）→ 哪儿材料多（5 条泳道，**块宽 ∝ 主归属张数**，极小条目 88px 保底并在图例披露）→ 复分标记怎么用。给要引用或评估这个库的人的三句可推翻的话与口径折扣，折进最后一块默认收起的 `<details>`。按 09-10 那份地图类调研（9 站实测）落地——**不做星图**，坐标由分片直接算出、永不重排；所有数字**从分片现算**不写死；窄屏退化成列表（390px 零溢出）。参考站与逐条对照（含**一处对规格的主动偏离**及其理由）记在 `D:\Git\tools\web-standard\inspiration\2026-09-10-map-references.md` §五。导航「看结构」组因此 14 → 15 个目的地。
...
- **分享与检索元信息（2026-09-14 补）**：20 个页面（含仓库根的重定向桩）都注入 `description` / `canonical` / `meta robots` / `og:*` / `twitter:*`，缩略图 `web/og.png`（1200×630，数字从 `meta.json` 现算生成）。**19 句描述逐页不同**——分享出去的那一行字是别人决定点不点开的全部依据，复制粘贴等于没有。新增离线判据 `scripts/check_page_meta.py`（接进 `validate_all.py`）：描述非空且互不相同、`og:url` 与文件名对得上、`og:image` 指向的文件**真的存在且是 1200×630**。
  - **没有放 `robots.txt`，是故意的**：它只在**源站根**（`craeatzeven.github.io/robots.txt`）生效，放在项目子目录爬虫不会读；放一个不生效的文件等于假装做了事。改用确实生效的 `<meta name="robots">`。将来绑自有域名（根就是本站）再补。
  - **未做**：`card.html` / `entry.html` 这类带参数页面的**逐条预览卡**——静态 HTML 拿不到具体是哪张卡，要做就得在构建期生成 1386 个静态页，属另一件事。
  - 仓库根 `index.html` 的 `?v=` 从 09-07 起就没跟上过（`bump_asset_version.py` 原来只扫 `web/*.html`）——已一并修，并把根页纳入该脚本。
- **顶栏导航重排（2026-09-14）**：原来是 10 项平铺，查过 git —— 那 10 项是**分 9 天、至少 8 个提交**逐项加上去的（建站进度条，不是信息架构），而且 `分类条目 / 主题浏览 / 教育专题` 三个入口指向同一件事。现改为跟着首页已设计的三组走：**读内容 / 看结构 / 用起来**，共 **15 个目的地**全部有位，每项带一句说明；`主题浏览` 改名 `筛选浏览`、`541 篇故事` 改名 `故事索引`，去掉单独的「首页」项（品牌即首页）。见 `docs/review-disposition.md` §十二。
...
## 十、vault 层：这个仓库同时是一个 Obsidian 库（2026-09-16 起）

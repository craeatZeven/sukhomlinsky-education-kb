...
(function () {
  ...
  /* 版本戳（2026-09-19 加）
     ------------------------------------------------------------------
     为什么要它：15 个 JS 渲染页原来把脚本写成 <script src="kb.js">（**没有 ?v=**），
     浏览器交回旧缓存时，页面里的新渲染器会调旧 kb.js 里不存在的 API（实测：
     「KB.plain is not a function」），于是页面在设完标题之后静默变成空壳 ——
     用户看到的就是「美育与自然下面什么都没有了」，而新无痕窗口永远复现不出来。
     现在：① 所有页面的 kb.js/theme.js/scene.js/style.css 都带同一个 ?v=；
           ② theme.js 在 DOMContentLoaded 时核对这个戳，对不上就把话说出来（见 theme.js）。 */
  KB.V = "20260919v16";
  window.KB = KB;

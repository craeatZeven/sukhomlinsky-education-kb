---
id: sk-0343
primary: null
seealso: []
facets:
  - S12
  - S5
tax_tags: []
aliases:
  - sk-0343
  - 坚持不懈的小蚂蚁
type: case
title: "坚持不懈的小蚂蚁"
lang: zh-CN
topics:
  - labor-education
  - learning-difficulties
source: singing-feather
ref: "The Singing Feather (Raduga Publishers, James Riordan trans.), The Persistent Ant, local txt lines approx 506–541"
url: ""
status: reviewed
created: "2026-09-07"
updated: "2026-09-07"
reviewed_by: "maintainer (source-checked)"
---

# Card sk-0343 — 坚持不懈的小蚂蚁

## 原文/Excerpt

> You’ve picked up the seed a thousand times,’ the dragon-fly said. ‘You must be tired by now. You’re not going to manage it anyhow. Why don’t you give up and let it be?’
>
> ‘You only give up when you fail,’
>
> That is how persistent is the ant.

## 中文转述/说明

小黑蚂蚁背着一粒罂粟籽回家，路上看见更大更香的南瓜籽，就放下小种子去搬大种子。南瓜籽一次次从背上滑下来，蜻蜓在旁边笑话它："你都搬了一千次了，反正也搬不动，放弃吧。"蚂蚁回答："只有失败了才放弃。"它再试一次，把种子稳稳背回了家。故事说明：真正的放弃只发生在失败之后；一次次的滑落不是失败，只是还没成功。

## 教育场景/应用

适用于孩子学新本领反复做不好、想放弃的时刻——写字、系鞋带、骑自行车、解难题等。讲蚂蚁的故事，把"又掉了"重新定义为"还没成功"，并和孩子约定：只有真正不干了才算输，只要还在试就不算输。

## 出处核对

- source: singing-feather
- ref: "The Singing Feather (Raduga Publishers, James Riordan trans.), The Persistent Ant, local txt lines approx 506–541"
- 是否已核对原文：是（已对照本地 OCR 文本；OCR 行断/缺页已注明）
